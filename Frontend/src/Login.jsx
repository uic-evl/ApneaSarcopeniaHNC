...
export default function Login() {
  ...
  const [fitbitCode, setFitbitCode] = useState(
    localStorage.getItem("fitbit-code")
  );
  const [whithingsCode, setWhithingsCode] = useState(
    localStorage.getItem("whithings-code")
  );
  const [fitbitToken, setFitbitToken] = useState(
    localStorage.getItem("fitbit-token")
  );
  const [whithingsToken, setWhithingsToken] = useState(
    localStorage.getItem("whithings-token")
  );
  ...
  async function fetchFitbitToken(code, force = false) {
    if ((!force && fitbitToken !== null) || fitbitCode === null) {
      return;
    }
  ...
  async function fetchWhithingsToken(code, force = false) {
    // Prepare the body for the POST request
    if ((!force && whithingsToken !== null) || whithingsCode === null) {
      return;
    }
    const body = new URLSearchParams();
  ...
  function enter() {
    if (
      localStorage.getItem("fitbit-token") !== null &&
      localStorage.getItem("whithings-token") !== null
    ) {
      navigate("/vis");
    }
  }
  ...
  return (
    ...
    >
      {!fitbitLoggedIn ? (
        <Button onClick={handleFitbitClick}>{"Log in with FitBit"}</Button>
      ) : (
        <Button onClick={handleFbLogoutClick}>{"Log Out FB"}</Button>
      )}
      {!withingsLoggedIn ? (
        <Button onClick={handleWithingsClick}>{"Log in with Whithings"}</Button>
      ) : (
        <Button onClick={handleWthLogoutClick}>{"Log Out Whithings"}</Button>
      )}
