...
export default function GaugeChart({ score, colorScale }) {
    ...
    useEffect(() => {
        ...
        const radius = Math.min(height - topMargin - bottomMargin, width / 2 - sideMargin);
        const thickness = radius * .15;
        svg.append('path')
            .attr('class', 'basePath')
    ...
    useEffect(() => {
        if (score === undefined || svg === undefined) { return }
        ...
        svg.append('text').attr('class', 'centerText')
            .attr('text-anchor', 'middle')
            .attr('x', width / 2).attr('y', .8 * height - bottomMargin)
