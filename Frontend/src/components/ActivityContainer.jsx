...
export default function ActivityContainer({ activityData, dateRange, stepsData, goalsDaily }) {
    const [activityPlotVar, setActivityPlotVar] = useState('totalActivity');

    const plotActivityOptions = [
        'activityCalories', 'totalActivity', 'minutesLightlyActive', 'minutesFairlyActive', 'minutesVeryActive', 'steps'
    ]
    const handleActivityButtonChange = ({ target: { value } }) => {
        if (activityData !== null && activityData[value] === undefined && value !== 'totalActivity' && value !== 'steps') {
